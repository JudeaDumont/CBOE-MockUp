...
public class OutputTopicTest {
	...
	public void testTopologyLogic() {
		...
		inputTopic.pipeInput(1, "test2", 11000L);
		inputTopic.pipeInput(2, "test3", 21000L);
		inputTopic.pipeInput(2, "test4", 31000L);
		inputTopic.pipeInput(3, "test5", 41000L);

		Awaitility.waitAtMost(Duration.ofSeconds(5)).until(() -> outputTopic.getQueueSize() > 0L);
		List<String> test1 = new LinkedList<>();
		test1.add("test1");
		test1.add(null);
		test1.add("test1test2");
		test1.add("test3");
		test1.add(null);
		test1.add("test3test4");
		test1.add("test5");
		assertThat(outputTopic.readValuesToList()).isEqualTo(test1);
	}
}
