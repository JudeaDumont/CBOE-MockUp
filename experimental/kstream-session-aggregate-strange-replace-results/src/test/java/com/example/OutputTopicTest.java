...
public class OutputTopicTest {
	...
	public void testTopologyLogic() {
		...
		Awaitility.waitAtMost(Duration.ofSeconds(5)).until(() -> outputTopic.getQueueSize() == 7L);
		List<String> expected = new LinkedList<>();
		expected.add("test1");
		expected.add(null);
		expected.add("*t*e*s*t*1*test2");
		expected.add("test3");
		expected.add(null);
		expected.add("*t*e*s*t*3*test4");
		expected.add("test5");
		assertThat(outputTopic.readValuesToList()).isEqualTo(expected);
	}
}
