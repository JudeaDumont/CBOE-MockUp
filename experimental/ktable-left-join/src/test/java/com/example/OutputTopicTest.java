...
public class OutputTopicTest {
	...
	public void testTopologyLogic() {
		...
		Awaitility.waitAtMost(Duration.ofSeconds(5)).until(() -> outputTopic.getQueueSize() == 8L);
		assertThat(outputTopic.readValuesToList()).isEqualTo(List.of("anull", "bnull", "cnull", "gnull", "hnull", "ad", "be", "cf"));

		inputTopicX.pipeInput(7, "j", 7001L); //close the window

		Awaitility.waitAtMost(Duration.ofSeconds(1));

		assertThat(outputTopic.getQueueSize()).isEqualTo(0L);
	}
}
